...
final class CodexServiceIncomingCommandExecutionTests: XCTestCase {
    ...
    func testLegacyCommandWorkdirRebindsThreadProjectPath() {
        let service = makeService()
        let threadID = "thread-\(UUID().uuidString)"
        let turnID = "turn-\(UUID().uuidString)"
        let callID = "call-\(UUID().uuidString)"
        let originalPath = "/Users/lipan"
        let newPath = "/Users/lipan/Desktop/lil-agents-clean-project"

        service.upsertThread(CodexThread(id: threadID, cwd: originalPath))
        ...
        service.handleNotification(
            ...
            params: .object([
                ...
                "msg": .object([
                    ...
                    "call_id": .string(callID),
                    "turn_id": .string(turnID),
                    "workdir": .string(newPath),
                    "command": .array([.string("pwd")]),
                ]),
            ])
        )

        XCTAssertEqual(service.thread(for: threadID)?.gitWorkingDirectory, newPath)
    }

    func testLegacyCdCommandRebindsThreadProjectPathWithoutGitHint() {
        let service = makeService()
        let threadID = "thread-\(UUID().uuidString)"
        let turnID = "turn-\(UUID().uuidString)"
        let callID = "call-\(UUID().uuidString)"
            ...
            params: .object([
                ...
                "msg": .object([
                    ...
                    "command": .array([
                        .string("/bin/zsh"),
                        .string("-lc"),
                        .string("cd /Users/lipan/Desktop/lil-agents-clean-project"),
                    ]),
                ]),
            ])
        )
