...
// Depends on: SwiftUI, AVFoundation, PhotosUI, CoreImage
...
struct QRScannerView: View {
    ...
    @State private var isDecodingPhotoQRCode = false
    @State private var selectedQRCodePhotoItem: PhotosPickerItem?
    ...
    var body: some View {
        ...
        .onChange(of: selectedQRCodePhotoItem) { _, item in
            guard let item else {
                return
            }
            Task {
                await decodeQRCode(from: item)
            }
        }
    ...
    private var photoQRCodePickerButton: some View {
        PhotosPicker(
            selection: $selectedQRCodePhotoItem,
            matching: .images,
            photoLibrary: .shared()
        ) {
            Label(isDecodingPhotoQRCode ? "Reading QR..." : "Choose QR Screenshot", systemImage: "photo.on.rectangle")
                .font(AppFont.subheadline(weight: .semibold))
                .foregroundStyle(.white)
        ...
        .buttonStyle(.plain)
        .disabled(isDecodingPhotoQRCode)
        .accessibilityLabel("Choose QR screenshot")
    }
    ...
    @MainActor
    private func decodeQRCode(from item: PhotosPickerItem) async {
        isDecodingPhotoQRCode = true
        defer {
            isDecodingPhotoQRCode = false
            selectedQRCodePhotoItem = nil
        }

        do {
            guard let data = try await item.loadTransferable(type: Data.self),
                  let image = UIImage(data: data),
                  let code = Self.decodeQRCode(from: image) else {
                scannerError = "No QR code was found in that screenshot."
                return
            }

            handleScanResult(code) {}
        } catch {
            scannerError = "Could not read that screenshot. Try saving the QR image again."
        }
    }

    private static func decodeQRCode(from image: UIImage) -> String? {
